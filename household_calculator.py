...
def collect_household_data(data, generate_household_id):
    ...
    selected_negeri = st.selectbox('NEGERI', ["Pilih"] + list(data['NEGERI'].unique()), key='NEGERI')
    ...
    if selected_negeri != "Pilih":
        ...
        selected_daerah = st.selectbox('DAERAH', ["Pilih"] + daerah_options, key='DAERAH')
    else:
        selected_daerah = st.selectbox('DAERAH', ["Pilih"], key='DAERAH')
    ...
    if selected_daerah != "Pilih":
        ...
        selected_strata = st.radio('STRATA', strata_options, key='STRATA')
    else:
        selected_strata = st.radio('STRATA', ["Pilih"], key='STRATA')
            ...
            if (umur_ksh != "Pilih" and jantina != "Pilih" and selected_negeri != "Pilih" and 
                ...
                if not filtered_data.empty:
                    ...
                    if umur_ksh in ["18-29 TAHUN", "30-59 TAHUN", "60 TAHUN"]:
                        st.markdown("<h3 style='font-size:18px;'>Masukkan Pendapatan</h3>", unsafe_allow_html=True)
                        ...
                        pendapatan_dari_harta_pelaburan = st.number_input('PENDAPATAN DARI HARTA PELABURAN', min_value=0, value=0, step=1, key=f'PENDAPATAN_HARTA_PELABURAN_{household_id}')
                        pindahan_semasa = st.number_input('PINDAHAN SEMASA', min_value=0, value=0, step=1, key=f'PINDAHAN_SEMASA_{household_id}')

                        total_income = (pendapatan_bergaji + pendapatan_bekerja_sendiri + pendapatan_dari_harta_pelaburan + pindahan_semasa)
...
# Load the CSV file
data = load_data('path_to_csv_file.csv')

# Function to generate unique household ID
generate_household_id = lambda: str(uuid.uuid4())

# Collect household data
household_data = collect_household_data(data, generate_household_id)

# Display the collected data
st.write("Collected Household Data:", household_data)
